...
- Static HTML, CSS and JavaScript. `npm run build` copies only public website assets into `dist/`, content-hashes every JS and CSS file (`app.a121135a.js`) and rewrites the references in the copied HTML, CSS and JS; source files stay unhashed for `npm run dev`. Hashed assets are served `immutable` (`.htaccess` on Hostinger, `vercel.json` on Vercel) while HTML stays `no-cache, must-revalidate`. **Deploy Hostinger from `dist/`** (including the hidden `.htaccess`), never from the source tree. Source, database files and local credentials are excluded from the build. The site is an installable PWA: `manifest.webmanifest` plus `sw.js` (registered by `pwa.js` on both pages, never fingerprinted; the build injects the guest shell's precache list) serve fingerprinted assets cache-first and HTML network-first, keep a paid gallery's previews for offline revisits (never originals, never `/api/*` otherwise, never the studio's routes), show an offline banner, and wake the studio's resume flow through Background Sync where supported.
...
2. Select a valid JPG, PNG or WebP selfie up to 10 MB (or drag one onto the box on desktop) and consent to server processing. Guest searches are capped per client (8 per 10 minutes, 30 per day — keyed per IPv4 address or per IPv6 /64) once migration 0010 is applied; over the cap the Worker answers 429 with `retry-after`, which the page honours with a countdown. The quota and the crew-token checks fail open only when their table is missing; any other D1 error answers 503 (guests) or 401 (crew).
3. Search; cancel or retry if needed.
4. Browse temporary watermarked previews as an uncropped contact sheet (tiles use the stored photo dimensions), enlarge photos (swipe / arrow keys / double-tap to zoom), filter favourites, hide a wrong match with **Not me** (`POST /api/searches/:id/hide`; the photo leaves the search on every later payload), and share the session title and site link to WhatsApp (Web Share, `wa.me` fallback — never a preview link or a token). Zero matches offer a second chance: a board-colour search (`POST /api/searches/:id/colour`, ranked over the stored clothing/board histograms, capped like `/api/match`) and a notify-me number (`POST /api/searches/:id/notify`). The landing page shows "Today's session lands by …" from `nextDropAt` and the results header shows the session's conditions chips when the crew filled them in. Favourites are held only in page memory and reset on a new search or reload. Signed preview links are refreshed automatically while the search is valid; after payment the browser keeps a 30-day gallery token in `localStorage` (`mjGallery`) so a guest can reopen their originals, and each original has a **Download** link (`?download=1` serves it as an attachment). The paid page also offers **Send my gallery link to WhatsApp** — the 30-day link is `/?gallery=<searchId>.<galleryToken>` and is a bearer link (anyone holding it can open the originals until it expires); the crew's Support tab can mint a fresh one (`POST /api/admin/searches/:id/resend`).
...
Open `/admin.html` (or `/admin` on Vercel) and sign in with your crew name, password and, once your authenticator is on, a 6-digit code (migration 0016: per-crew accounts with PBKDF2 passwords and RFC 6238 TOTP, roles `photographer` and `admin` — a photographer cannot refund, grant free unlocks, delete a published session or manage accounts; every delete, publish, refund, grant, sign-in and account change lands in the audit log, visible in the **Crew** pane). The shared `ADMIN_PASSWORD` keeps working while no account exists or while the Worker var `LEGACY_SHARED_LOGIN` is `true`; five failed attempts per IP **or per account name** lock sign-in for 15 minutes. Then create a session, upload images, and publish. Uploads accept JPG, PNG, WebP and iPhone HEIC/HEIF files; HEIC is decoded in the browser before upload, which Safari and macOS do natively, while other browsers report a per-file error for that photo and continue with the rest. A retried upload attempt is sent with `onDuplicate=skip`, so a lost response never stores a photo twice. When the Worker has the R2 API secrets (`R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`) and the bucket's CORS policy allows `PUT` from the site origins, originals go **direct to R2** through a presigned URL (`POST /api/admin/sessions/:id/uploads/presign`, SigV4 signed by the Worker, 15 min) and the studio then registers the photo with its preview (`…/uploads/complete`); without the secrets `presign` answers 503 and the batch streams through the Worker as before. Previews and thumbs are WebP where the browser can encode it (JPEG otherwise; the Worker sniffs the bytes). An interrupted batch is remembered in IndexedDB (names, sizes and timestamps only) and the studio offers **Resume publishing?** on the next visit; **Regenerate previews** on a session card rebuilds every preview and thumb from the originals without re-uploading them (`PUT /api/admin/photos/:id/preview`). Uploads otherwise use the streaming Worker path (`UPLOAD_STREAMING` in `admin.js`) and send a 320px watermarked grid thumbnail per photo after the original (previews are 600px on the long edge; both are rendered off the main thread by `preview-worker.js`, with an identical main-thread fallback in `admin.js` for browsers without `OffscreenCanvas` or for HEIC files the worker cannot decode) (`POST /api/admin/photos/:id/thumb`); thumbnails are optional and only exist for photos uploaded after 0008. Landing-page cover links carry a `cover` token that renders but cannot be downloaded; per-photo download events name the paid search. Session cards show a money strip (searches · unlocks · rupees) from `GET /api/admin/stats` once that route is deployed. In the **Review** tab, `Z` or the Undo button puts the last Same/Different decision back within 10 minutes (`POST /api/admin/undo-review`); each card shows a confidence meter and the full frames beside the crops. In the photo grid, tiles can be multi-selected for **Re-index / Set cover / Move to… / Delete** (`POST /api/admin/photos/bulk`, at most 200 ids per call; on an older Worker delete and re-index fall back to the per-photo routes). Publishing asks for a cover first once migration 0015's Worker is live (`409 { needsCover: true }`; "Publish without a cover" sends `noCover: true`). Sessions carry **conditions** (break, swell ft, wind, tide, photographer, next drop time) on create and edit, pre-filled from the first photo's EXIF; cards show queue depth, ETA and grouped failure reasons from the dashboard's `indexing` block. The **Money** tab shows the per-session funnel (`GET /api/admin/stats`), settlements and unreconciled payments (`GET /api/admin/settlements`, Cashfree settlements + recon APIs) and refunds (`POST /api/admin/payments/:id/refund`, Cashfree Create Refund; `REFUND_STATUS_WEBHOOK` updates the row). The **Support** tab looks a guest up by phone, order id or search id (`GET /api/admin/lookup`, phone masked), resends the 30-day link, grants a free unlock (`POST /api/admin/searches/:id/grant`, recorded in `grants`, excluded from rupees) and shows what the guest saw. The crew can manage sessions, inspect photos, reindex and review borderline face pairs, and set the photo-pack price guests pay to unlock originals. **Upload more** on a session card adds photos to a draft or published session; files whose names already exist in that session are listed first, and the crew chooses whether to replace the existing photos, upload only the new files, or keep both as numbered copies (`IMG_0412-2.jpg`). Reviewing face pairs records a crew decision; those decisions are not currently applied to guest match scoring.
...
```sh
npm run check
npm run build          # fingerprints assets into dist/ and verifies every reference resolves
npm test
npx wrangler deploy --dry-run
npm run images         # re-export the hero srcset variants from assets/brand-surf-wide.webp (sharp)
npm run lighthouse     # Lighthouse CI budgets (lighthouserc.json) against a dev server on port 4190; needs Chrome (CHROME_PATH)
npm run e2e            # Playwright (Chromium) end-to-end, screenshot and axe suite; every /api call is mocked, nothing reaches production; `npm run e2e -- --update-snapshots` re-baselines and also creates the set on a platform that has none (baselines are per platform under tests/e2e/__screenshots__/)
npm run verify         # check + test + build + wrangler dry-run + e2e — what CI (.github/workflows/ci.yml) runs
npm run accuracy -- --selftest   # matching-accuracy tool self-test (docs/accuracy.md)
npm run backup -- --dry-run      # nightly D1 → R2 backup script (runs for real from .github/workflows/backup.yml)
npm run restore-drill -- <dump>  # loads a dump into a fresh LOCAL database and runs the sanity queries
```

Operations — deploy order, secrets, rollbacks, the crew lockout reset, backups (nightly `wrangler d1 export` to `r2://mambo-jambo-photos/backups/d1/`, 30-day daily retention plus monthly copies), the recorded restore drill and the stall/health alerting cron — are in `docs/runbook.md`.
...
1. Verify D1 schema compatibility and private R2 storage. Apply `migrations/0002_cashfree_payments.sql`, `migrations/0004_capture_metadata.sql`, `migrations/0005_photo_links.sql`, `migrations/0006_photo_appearances.sql`, `migrations/0007_match_learning.sql`, `migrations/0008_thumbnails_and_login_throttle.sql`, `migrations/0009_session_cover.sql`, `migrations/0010_rate_limits.sql` (guest search quota), `migrations/0011_admin_sessions.sql` (revocable crew tokens), `migrations/0012_photo_dimensions.sql` (`photos.width/height`, parsed from the original's JPEG/PNG/WebP header at upload and returned on every photo object, `null` for older photos) `migrations/0013_events.sql` (funnel `events` — kinds `search`, `match`, `zero_match`, `checkout`, `paid`, `download`; stores only kinds, session/search ids and timestamps, never selfies, phones or IPs — plus `match_feedback.subject_id` for undo), `migrations/0014_session_conditions.sql` (six session-conditions columns), `migrations/0016_crew_accounts.sql` (`crew_users`, `audit_log`, `admin_sessions.user_id/role` — apply once, before the Worker) and `migrations/0015_support.sql` (`match_hides`, `notify_requests`, `refunds`, `grants`, `searches.colour_photo_ids_json`, `searches.gallery_link_expires_at`, `payments.customer_phone`) if upgrading an existing database, one file at a time, **before** deploying the Worker (it caches column presence per isolate). `GET /api/health` reports `migrations: { adminSessions, rateLimits, photoDimensions, events, sessionConditions, support, crewAccounts }` so you can confirm they landed. `wrangler.jsonc` now carries a `*/10 * * * *` cron that pings the face service warm and a queue consumer `max_concurrency` of 3. The Worker degrades gracefully before they do (no search cap; signature-only crew tokens with a logged warning). The Worker detects `photos.thumb_key` and `login_attempts` at runtime, so it can be deployed before 0008 is applied — grid tiles fall back to the 1400px preview and sign-in throttling is skipped until then.
2. Configure Worker secrets `ADMIN_PASSWORD`, `SESSION_SECRET`, `CASHFREE_APP_ID`, `CASHFREE_SECRET_KEY`, optionally `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY` (direct-to-R2 uploads), `ALERT_WEBHOOK_URL` and/or `RESEND_API_KEY` + `ALERT_EMAIL_TO` (the ten-minute cron's stall/health alerts; without a sink it only logs), optionally `FACE_API_KEY` (sent to the face service as `x-face-key`; set the same value as a secret in the Hugging Face Space **after** the Worker is deployed with it, and the Space only enforces it once its own variable exists), and variables `ALLOWED_ORIGIN`, `FACE_API_URL`, `MATCH_THRESHOLD`, `CASHFREE_ENV` (`sandbox` or `production`). `BURST_GAP_SECONDS` (default 2) and `APPEARANCE_THRESHOLD` (default 0.85) are optional and only affect fallback-link candidate generation.
3. In the Cashfree dashboard, point the webhook URL at `<worker-url>/api/payment/webhook` (version `2023-08-01` or later) and subscribe it to `PAYMENT_SUCCESS_WEBHOOK` **and** `REFUND_STATUS_WEBHOOK`. The Worker refuses deliveries whose `x-webhook-timestamp` is more than 5 minutes off. Cashfree signs webhooks with the same `CASHFREE_SECRET_KEY` used for API calls — there is no separate webhook secret to configure.
4. Deploy the Worker with `npm run deploy:api`. Crew tokens now carry a server-side session id (`admin_sessions`), so every crew member signs in again once after this deploy; **Sign out** calls `POST /api/admin/logout`, which revokes the token immediately. Cashfree webhooks older or newer than 5 minutes (`x-webhook-timestamp`, seconds or milliseconds) are refused as replays before the body is read (bodies are capped at 64 KB), and a `PAYMENT_SUCCESS_WEBHOOK` whose payment is still `PENDING` is acknowledged without unlocking.
...
`GET /api/health` on the Worker is public and uncached (`cache-control: no-store`). It returns `{ ok, checks: { db, r2, face }, migrations: { adminSessions, rateLimits }, time }` with HTTP 200 when every check is `ok` and 503 otherwise: `db` runs `SELECT 1` on D1, `r2` performs a `head()` on the photo bucket (a missing object is fine; only an R2 error fails), and `face` is `skipped` unless `?deep=1` is passed, in which case the face service must answer a `HEAD` request within 3 s — any HTTP status counts, including 405, since the point is reachability, not inference. Checks run independently, so one failure never hides another, and the response never carries error text or configuration values. The admin studio's topbar health pill polls this endpoint and shows API, R2 and face-service status; an uptime monitor can watch the same URL.
...
- Matching precision/recall on a consented session: the procedure and tool exist (`docs/accuracy.md`, `node scripts/accuracy.mjs`, `--selftest` runs a synthetic fixture against the real `/api/match` ranking) but have **not** been run on real crew labels; `MATCH_THRESHOLD` stays 0.62 until then.
