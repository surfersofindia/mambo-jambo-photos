...
**Follow-up, same day: v1.0.4 — stale offline banner.** Reported live by the user right after the v1.0.3 deploy: the "You're offline" banner showed on a device that was actually online, and persisted through a hard reload (expected — nothing had shipped yet at that point). Root cause: `pwa.js`'s banner only recomputed on the browser's `online`/`offline` events, which are unreliable across a backgrounded tab — a brief Wi-Fi/cellular handover can fire `offline` while hidden and never fire `online` back. Fixed by also re-checking `navigator.onLine` on `visibilitychange` (tab regains focus) and `pageshow` (bfcache restore); `tests/pwa.test.mjs`'s Chromium-driven offline test and the full `npm run verify` stayed green. Released as `v1.0.4` (commit `51f3320`), deployed to both hosts the same way as v1.0.3 (Hostinger TUS + cache clear, Vercel `--scope mambo-jambo1`); confirmed live via a fresh Playwright load — `pwa.9db8e41f.js` (the fixed, newly-hashed file) serving, banner correctly hidden, `version.json` on both hosts reads `1.0.4`.
